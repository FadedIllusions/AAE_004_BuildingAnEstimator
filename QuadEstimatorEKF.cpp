...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;  // yaw
  ...
  Quaternion<float> est_attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  est_attitude.IntegrateBodyRate(gyro,dtIMU);
  
  float predictedRoll = est_attitude.Roll();
  float predictedPitch = est_attitude.Pitch();
  ekfState(6) = est_attitude.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(0) += dt * curState(3);  // x += x_dot * dt
  predictedState(1) += dt * curState(4);  // y += y_dot * dt
  predictedState(2) += dt * curState(5);  // z += z_dot * dt

  V3F att_vec = attitude.Rotate_BtoI(accel); 

  predictedState(3) += dt * att_vec.x;  // x_dot += a_x * dt
  predictedState(4) += dt * att_vec.y;  // y_dot += a_y * dt
  predictedState(5) += dt * (att_vec.z - 9.81f); // z_dot += a_z * dt - gravity
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
 ...
 // hPrime in row echelon form with last column 0
 for(int i = 0; i < 6; i++)
 {
    hPrime(i,i) = 1;
 }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float z_error = magYaw - ekfState(6);
